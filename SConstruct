...
from gdextension import SHLIB_AFFIXES, generate as generate_gdextension, library_filename, verify_shlib_affixes
...
opts.Add(BoolVariable(
    ...
    "(implied by every template, and by every platform but windows)", False))
...
# that also implements the runtime subset of the vh_* ABI. The UE host exists only for Windows, and
# verse_host.cpp compiles its DLL loader out everywhere else, so every other platform always needs
# vm/ built in; a Windows editor has it only with verse_vm=yes, because an editor always uses the
# host. VERSE_VM_STATIC is what lets src/ fill VerseHostLibrary directly from vm/'s functions
# instead of resolving them with GetProcAddress (src/verse_host.cpp's load_static).
# VERSE_HOST_IMPLEMENTATION is deliberately never defined here: that would export the vh_* symbols
# from this library with dllexport, which only tools/build_verse_vm.py's standalone DLL wants.
#
# Every template carries it, Windows' included: it costs ~565 KiB as of T5.4 (measured: 2,969,600
# bytes without it, 3,548,160 with -- vm/ is still growing, so treat the delta as approximate) and
# changes nothing about the host backend, which VERSE_VM_STATIC only sits beside on Windows. One
# template this way carries both backends (T5.3), so `scons target=template_release` alone is
# enough for an export to choose either at `verse/runtime/backend`.
verse_vm = bool(env["verse_vm"]) or env["platform"] != "windows" or env["target"] != "editor"
if verse_vm:
...
# godot-cpp decides the prefix only for Windows; everywhere else it is the build machine's default,
# so web built on Linux would be libgodot-verse.wasm and android built on Windows godot-verse.so.
if env["platform"] in SHLIB_AFFIXES:
    env["SHLIBPREFIX"] = SHLIB_AFFIXES[env["platform"]][0]
