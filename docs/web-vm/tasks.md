...
| ID | Task | Room / model | Depends | Check | Status |
| --- | --- | --- | --- | --- | --- |
| T5.1 | The rest of the runtime `vh_*` subset in `vm/` (fields, callbacks, `vh_tick`, statics, exports, signals, garbage collection), and the file reader `src/` sets over `FileAccess` | clean / opus | T1.11, M4 | a headless run of a one-script project on the vm backend prints from `_Ready` | done |
| T5.2 | The other 45 Godot natives | clean / opus | T1.10, T5.1 | integration cases touching each native pass | done |
...
| T5.4 | Export plugin: ships `program.vbc`; omits the host DLL and `tbbmalloc.dll` on the vm backend | clean / sonnet | T5.3 | an export on the vm backend has no DLL beside the executable, and ships no `program.vbc.report.txt` (the writer puts it in the cook directory) | done |
| T5.5 | Export layer runs on both backends with named counts; fix until green | clean / opus | T5.2, T5.4 | `run_tests.py --only export` passes on both | done: 519 passed, 0 failed, 11 skipped on each |
