...
[`by-hand-checklist.md`](by-hand-checklist.md), which had never been run. Twenty of its
twenty-one entries have now been watched happen; one — `_CanDropData` — is still owed, and §B11
says why it is the only one that could not be reached another way.

Two kinds of entry are below. **B1–B9** are what the session saw go wrong, each traced to the code
that causes it rather than left as a symptom. **B10–B11** are what the session learned about the
checklist itself: one entry did not need to be on it, and one belongs there permanently.
...
## B1. Override completion is dead in a class body, and has been since Phase 4
...
## B2. `_Input` is not broken; the parameter name was
...
## B3. `_make_function` writes a stub that does not compile
...
## B4. A connected handler gets no gutter icon, and there are two reasons
...
## B5. "No suitable template", on a dialog that then writes a template
...
## B6. The template becomes a direct translation of GDScript's
...
## B7. `explain_effect_errors` goes; the other two appenders stay
...
## B8. A `@tool` script only takes effect after an editor restart
...
## B9. A runtime error's `ERROR:` line runs four fields together
...
## B10. `_HasPoint` *is* reachable headless, and G19 is half retired
...
## B11. `_CanDropData` stays on the by-hand list, and this is why
...
B1 first, and not because it is the largest — because it is the one the session kept paying for.
Then B3 and B4 together, since B3 is a prerequisite for checking B4. B5 and B6 are the same file and
one sitting. B7 is a deletion. B9 is a format string. B8 is the only entry here that is a design
question as much as a fix, and the only one that needs new state on `VerseScript`.

B10 is a test to write; B11 is a line to leave alone.
