...
- **R-PLAT-3 (SHOULD)** Web export runs Verse. Status: **done** (Phase 7.5), on a second execution
  ...
  (`verse-on-web.md`). `verse/runtime/backend` defaults to it on Web through its `.web` feature override, and a Web export
  ships no UE binary.
...
- **R-PLAT-4 (MUST)** A platform that is not supported fails at export time with a clear message,
  ...
  _export_begin` refuses `android` and `ios` with one sentence naming the platform; `web` exports
  only on the interpreter and is refused with a sentence when the preset resolves
  `verse/runtime/backend` to `host` (Phase 7.5). It also
