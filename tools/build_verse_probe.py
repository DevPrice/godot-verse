#!/usr/bin/env python3
"""Compiles the Verse probe with MSVC.

Not part of run_tests.py: the probe has no expected output and asserts nothing. It compiles whatever
.verse files it is given and prints what the compiler said, which is how a language question gets an
answer that was run rather than read (docs/phase-4-design.md 2).
"""
...
def main() -> None:
    ...
    repo = repo_root()
    src = repo / "tests" / "verse_probe" / "verse_probe.cpp"
    include_dir = repo / "include"
    out_dir = repo / "bin"
    out_exe = out_dir / "verse_probe.exe"
    ...
    print(f"[build_verse_probe] running: {cl_cmd}")
    ...
    print(f"[build_verse_probe] built {out_exe}")
