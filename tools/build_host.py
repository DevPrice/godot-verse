...
BUILD_ID_NAME = "host_build_id.gen.h"


def host_source_digest(staged: dict[str, Path]) -> str:
    """A digest of exactly the files this build stages, keyed by their staged path.

    This is the identity the stamp compares, and it is content rather than `HEAD` because `HEAD`
    answers a different question: a doc commit changed the stamp, which relinked all three host
    targets and refused every cook taken before it, while an uncommitted edit to `host/` changed
    nothing and let a stale cook load. What decides whether a cook and a runtime host agree is the
    source they were built from, which is this set.
    """
    digest = hashlib.sha256()
    for rel in sorted(staged):
        digest.update(rel.encode("utf-8"))
        digest.update(b"\0")
        digest.update(hashlib.sha256(staged[rel].read_bytes()).digest())
    return digest.hexdigest()


def write_build_id(engine: Path, repo: Path, staged: dict[str, Path]) -> Path:
    """The build identity every host binary carries, so a cook and a host can be told apart.

    An exported game's Verse data is cooked by verse_cook.exe and read back by
    verse_host_runtime.dll, and nothing in the data says which build produced it: the sidecar
    carries these two strings and the runtime host compares the host id against its own (D6).
    Baked in rather than read from a file beside the binary, because a game directory is exactly
    where a file goes missing and a stamp that can be absent is not a stamp.

    Written into bin/ and staged into the host's Private/ by name; never committed, and every
    target that goes into one export is built from one checkout, so the two agree by construction.
    """
    fields = [
        ("VH_BUILD_HOST_ID", host_source_digest(staged)),
        ("VH_BUILD_ENGINE_COMMIT", git(engine, "rev-parse", "HEAD") or "unknown"),
    ]
    text = ("// Generated by tools/build_host.py. Not committed; not hand-edited.\n"
            "#pragma once\n\n"
            + "".join(f'#define {key} "{value}"\n' for key, value in fields))

    path = repo / "bin" / BUILD_ID_NAME
    path.parent.mkdir(parents=True, exist_ok=True)
    # Rewritten only when it changes: the timestamp is what mirror_host copies on, and a new one
    # every build would relink the host for nothing.
...
def main() -> None:
    ...
    staged_extra[f"Private/{BUILD_ID_NAME}"] = write_build_id(
        engine, repo, collect_src_files(src) | staged_extra)

    copied, removed = mirror_host(src, dst, staged_extra)
