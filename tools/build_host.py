...
def write_build_id(engine: Path, repo: Path) -> Path:
    ...
    carries these two strings and the runtime host compares them against its own (D6). Baked in
    rather than read from a file beside the binary, because a game directory is exactly where a
    file goes missing and a stamp that can be absent is not a stamp.
    ...
    """
    fields = [
        ("VH_BUILD_GODOT_VERSE_COMMIT", git(repo, "rev-parse", "HEAD") or "unknown"),
...
def main() -> None:
    ...
    staged_extra = {
        ...
        f"Private/{BUILD_ID_NAME}": write_build_id(engine, repo),
    }
