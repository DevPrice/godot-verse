#!/usr/bin/env python3
"""Compiles the standalone host benchmark with MSVC.

Not part of run_tests.py: it reports timings rather than pass/fail, and R-PERF-2 asks for recorded
numbers rather than thresholds, so a slow machine must not be able to fail a build.
"""
...
def main() -> None:
    ...
    repo = repo_root()
    src = repo / "tests" / "host_bench" / "host_bench.cpp"
    include_dir = repo / "include"
    out_dir = repo / "bin"
    out_exe = out_dir / "host_bench.exe"
    ...
    cl_cmd = (
        f'call "{vcvars64}" && '
        f'cl /std:c++20 /EHsc /O2 /Zi /I"{include_dir}" "{src}" '
        f'/Fe"{out_exe}" /Fo"{out_dir}\\\\"'
    )
    print(f"[build_bench] running: {cl_cmd}")
    ...
    print(f"[build_bench] built {out_exe}")
