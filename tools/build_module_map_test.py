#!/usr/bin/env python3
"""Compiles the standalone Verse module-map test with MSVC. No godot-cpp, no SCons."""
...
def main() -> None:
    ...
    repo = repo_root()
    map_src = repo / "src" / "verse_module_map.cpp"
    test_src = repo / "tests" / "verse_module_map" / "verse_module_map_test.cpp"
    src_dir = repo / "src"
    out_dir = repo / "bin"
    out_exe = out_dir / "verse_module_map_test.exe"

    for path in (map_src, test_src):
        if not path.exists():
    ...
    cl_cmd = (
        f'call "{vcvars64}" && '
        f'cl /std:c++20 /EHsc /Zi /I"{src_dir}" "{map_src}" "{test_src}" '
        f'/Fe"{out_exe}" /Fo"{out_dir}\\\\"'
    )
    print(f"[build_module_map_test] running: {cl_cmd}")
    ...
    print(f"[build_module_map_test] built {out_exe}")
