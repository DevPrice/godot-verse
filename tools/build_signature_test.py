#!/usr/bin/env python3
"""Compiles the standalone Verse signature parser test with MSVC. No godot-cpp, no SCons."""
...
def main() -> None:
    ...
    repo = repo_root()
    signature_src = repo / "src" / "verse_signature.cpp"
    test_src = repo / "tests" / "verse_signature" / "verse_signature_test.cpp"
    src_dir = repo / "src"
    out_dir = repo / "bin"
    out_exe = out_dir / "verse_signature_test.exe"

    for path in (signature_src, test_src):
        if not path.exists():
    ...
    cl_cmd = (
        f'call "{vcvars64}" && '
        f'cl /std:c++20 /EHsc /Zi /I"{src_dir}" "{signature_src}" "{test_src}" '
        f'/Fe"{out_exe}" /Fo"{out_dir}\\\\"'
    )
    print(f"[build_signature_test] running: {cl_cmd}")
    ...
    print(f"[build_signature_test] built {out_exe}")
